...
export class TimelineService {
  ...
  hasOverlap(workCenterId: string, startDate: string, endDate: string, excludeDocId?: string): boolean {
    const orders = this.getWorkOrdersForCenter(workCenterId).filter(wo => wo.docId !== excludeDocId);
  ...
  dateToPixel(date: Date | string, startDate: Date, columnWidth: number, zoom: ZoomLevel): number {
    const d = typeof date === 'string' ? new Date(date) : date;
    const diffMs = d.getTime() - startDate.getTime();
    const msPerUnit = zoom === 'day' ? 86400000 : zoom === 'week' ? 86400000 * 7 : 86400000 * 30.44;
    return (diffMs / msPerUnit) * columnWidth;
  }

  pixelToDate(px: number, startDate: Date, columnWidth: number, zoom: ZoomLevel): Date {
    const msPerUnit = zoom === 'day' ? 86400000 : zoom === 'week' ? 86400000 * 7 : 86400000 * 30.44;
    const ms = (px / columnWidth) * msPerUnit;
